...
class ProcoreSpider(Spider):
    ...
    def __init__(self, **kwargs: Any):
        ...
        self.output_file_name = os.path.join(output_dir, f'procore_contractors {timestamp}.xlsx')

    def start_requests(self) -> Iterable[Request]:
        yield Request(url="https://www.procore.com/network/us", callback=self.parse, headers=self.headers)

    def parse(self, response, **kwargs):
        api_url = f"https://pcn.procore.com/api/search?lat=30.43977&lng=-84.28065&state={self.state}&country=USA&sort=relevance&page=1&pageSize=40"
        yield Request(api_url, callback=self.parse_pagination, headers=self.headers)
    ...
    def parse_pagination(self, response):
        ...
        for page_num in range(total_pages + 1):
            page_url = f"https://pcn.procore.com/api/search?lat=30.43977&lng=-84.28065&state={self.state}&country=USA&sort=relevance&page={page_num}&pageSize=40"
            yield Request(page_url, callback=self.detail_page, headers=self.headers)
    ...
    def detail_page(self, response):
        ...
        for contractor in data.get('results', []):
            ...
            self.write_to_xlsx(item)
            yield item

    def write_to_xlsx(self, data: OrderedDict) -> None:
        file_exists = os.path.exists(self.output_file_name)

        if not file_exists:
            wb = Workbook()
            ws = wb.active
            ws.title = "Procore Data"
            ws.append(list(data.keys()))  # Write header row
        else:
